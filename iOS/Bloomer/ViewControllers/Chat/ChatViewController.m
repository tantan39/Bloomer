...
- (void)viewDidLoad
{
    ...
    [socketManager setOnMessageReceive:^(message* data) {
        ...
        if ([newMessage.room_id isEqualToString:self.roomID]) {
            ...
            [weakSelf.emptyView setHidden:YES];
             [weakSelf.chatTableView setHidden:NO];
        ...
        if ([weakSelf.delegate respondsToSelector:@selector(chatViewController_UpdateListMessageWith:)]) {
            [weakSelf.delegate chatViewController_UpdateListMessageWith:newMessage];
        }
    ...
    [socketManager setOnBlockEventResponse:^(ChatStatus * status) {
        if (status.block == 2) { //Blocked
            chatStatus = status;
            [weakSelf checkBlockStateWithStatus:chatStatus];

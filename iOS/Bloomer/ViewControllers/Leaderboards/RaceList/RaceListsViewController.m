...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout referenceSizeForHeaderInSection:(NSInteger)section
{
    ...
    if (listCategory.count > 0)
    {
        ...
        for (races_list *race in sponsoredRaces) {
			CGFloat topBannerHeight = race.topBannerLink.length ? 100 : 0;
            sponsoredViewHeight += race.logo.length ? (screenWidth * 23 / 31) : screenWidth;
			sponsoredViewHeight += topBannerHeight;
        }
		
		CGFloat exclusiveViewHeight = 0;
		for (races_list *race in exclusiveRaces) {
			CGFloat topBannerHeight = race.topBannerLink.length ? 100 : 0;
			exclusiveViewHeight += screenWidth + topBannerHeight;
		}
        
        return CGSizeMake(screenWidth, bannerViewHeight + 684 + (sponsoredRaces.count > 0 ? sponsoredViewHeight : 0) + (exclusiveRaces.count > 0 ? 30 + exclusiveViewHeight : 0) + (themedRaces.count > 0 ? -10 : -30) + (exclusiveRaces.count > 0 ? 10 : 0) + (sponsoredRaces.count > 0 ? 10 : 0) );
    }
    else
    {
//        CGFloat screenWidth = [UIScreen mainScreen].bounds.size.width;

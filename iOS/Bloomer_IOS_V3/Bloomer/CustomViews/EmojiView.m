...
- (void)initEmojiKeyBoardImage:(UIView*)tabContentView animatedLine:(UIView*)animatedLine animatedLineLeftMargin:(NSLayoutConstraint*)animatedLineLeftMargin
{
    ...
    for (NSInteger i = 0; i < stickers.count; i++)
    {
        ...
        if (i == 0)
        {
            [self setupConstraintsForView:collectionView previousView:nil isFirstView:true parentView:self.scrollViewContentView width:self.scrollViewContentView.frame.size.width spacing:0 parentViewMargin:0];
        }
        else
        {
            UIView *previousView = [stickerCollectionViews objectAtIndex:i - 1];
            [self setupConstraintsForView:collectionView previousView:previousView isFirstView:false parentView:self.scrollViewContentView width:self.scrollViewContentView.frame.size.width spacing:0 parentViewMargin:0];
        }
        
        [collectionView reloadData];
    }
    
    self.scrollViewContentViewWidth.constant = self.scrollView.frame.size.width * stickers.count;

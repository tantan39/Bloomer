...
- (void)viewDidLoad
{
    ...
    if (self){
        ...
        if (self.image.size.height >= self.image.size.width) {
            _cropArea = CGRectMake(0, 0, self.image.size.width, self.image.size.width * 0.6);
        }
        else {
            _cropArea = CGRectMake(0, 0, self.image.size.height / 0.6, self.image.size.height);
        }
...
- (IBAction)done:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(ImageCropViewControllerSuccess:didFinishCroppingImage:rect:)])
    {
        UIImage *cropped;
        CGRect rect = self.cropView.cropAreaInImage;
        if (self.image != nil){
            cropped = [AppHelper cropToBounds:self.image rect:rect];
        }
        [self.delegate ImageCropViewControllerSuccess:self didFinishCroppingImage:cropped rect:rect];
    }
}

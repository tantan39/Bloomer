...
//    }
    ...
    {
        return NSDateTimeAgoLocalizedStrings(@"1 min");
    }
    else if (deltaMinutes < 60)
    {
        return [self stringFromFormat:NSDateTimeAgoLocalizedStrings(@"%%d %@mins") withValue:deltaMinutes];
    }
    else if (deltaMinutes < 120)
    {
        return NSDateTimeAgoLocalizedStrings(@"1 hr");
    }
    else if (deltaMinutes < (24 * 60))
    {
        minutes = (int)floor(deltaMinutes/60);
        return [self stringFromFormat:NSDateTimeAgoLocalizedStrings(@"%%d %@hrs") withValue:minutes];
    }
    else if (deltaMinutes < (24 * 60 * 2))
    {
        return NSDateTimeAgoLocalizedStrings(@"1 day"); //@"Yesterday"
    }
    else if (deltaMinutes < (24 * 60 * 7))
    {
        minutes = (int)floor(deltaMinutes/(60 * 24));
        return [self stringFromFormat:NSDateTimeAgoLocalizedStrings(@"%%d %@days") withValue:minutes];
    }
    else if (deltaMinutes < (24 * 60 * 14))
    {
        return NSDateTimeAgoLocalizedStrings(@"1 week"); // @"Last week"
    }
    else if (deltaMinutes < (24 * 60 * 31))
    {
        minutes = (int)floor(deltaMinutes/(60 * 24 * 7));
        return [self stringFromFormat:NSDateTimeAgoLocalizedStrings(@"%%d %@weeks") withValue:minutes];
    }
    else if (deltaMinutes < (24 * 60 * 61))
    {
        return NSDateTimeAgoLocalizedStrings(@"1 month"); // Last month
    }
    else if (deltaMinutes < (24 * 60 * 365.25))
    {
        minutes = (int)floor(deltaMinutes/(60 * 24 * 30));
        return [self stringFromFormat:NSDateTimeAgoLocalizedStrings(@"%%d %@months") withValue:minutes];
    }
    else if (deltaMinutes < (24 * 60 * 731))
    {
        return NSDateTimeAgoLocalizedStrings(@"1 year"); //Last year
    }
    
    minutes = (int)floor(deltaMinutes/(60 * 24 * 365));
    return [self stringFromFormat:NSDateTimeAgoLocalizedStrings(@"%%d %@years") withValue:minutes];
}
...
- (NSString *)dateTimeAgo
{
    ...
    else if (components.hour >= 1)   // up to 23 hours ago
    {
        ...
        return [self stringFromFormat:@"%%d %@hrs ago" withValue:components.hour];
    }
    else if (components.minute >= 1) // up to 59 minutes ago
    {
        ...
        return [self stringFromFormat:@"%%d %@mins ago" withValue:components.minute];
    }
...
//    return [self stringFromFormat:@"%%d %@seconds ago" withValue:components.second];
    return [self stringFromFormat:@"Now" withValue:components.second];

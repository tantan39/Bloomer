...
@implementation WinnersClubTableViewCell {
    MessagePopupRacesViewController *popup;
}

+ (NSString *)cellIdentifier {
    return @"WinnersClubTableViewCell";
}
...
- (void)awakeFromNib {
    ...
    self.avatarSquareView.hidden = TRUE;
    self.avatarRoundView.hidden = TRUE;
    self.cupImageView.layer.cornerRadius = self.cupImageView.frame.size.height / 2;
    self.cupUIview.layer.cornerRadius = self.cupUIview.frame.size.height / 2;
}

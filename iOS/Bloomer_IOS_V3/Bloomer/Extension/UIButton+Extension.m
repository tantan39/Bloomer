...
- (void)setIconEdgeInset{
    ...
    CGSize titleSize = [buttonTitle sizeWithAttributes:@{ NSFontAttributeName : [UIFont fontWithName:@"SFProText-Medium" size:12.f] }];
    UIImage *buttonImage = self.imageView.image;
    CGSize buttonImageSize = buttonImage.size;
    
    CGFloat offsetBetweenImageAndText = 8; //vertical space between image and text
    
    [self setImageEdgeInsets:UIEdgeInsetsMake((buttonSize.height - (titleSize.height + buttonImageSize.height)) / 2 - offsetBetweenImageAndText,
                                                         (buttonSize.width - buttonImageSize.width) / 2 - 10,
                                                         0,0)];
    [self setTitleEdgeInsets:UIEdgeInsetsMake((buttonSize.height - (titleSize.height + buttonImageSize.height)) / 2 + buttonImageSize.height + offsetBetweenImageAndText,
                                                         titleSize.width + [self imageEdgeInsets].left > buttonSize.width ? -buttonImage.size.width  +  (buttonSize.width - titleSize.width) / 2 - 10 : (buttonSize.width - titleSize.width) / 2 - buttonImage.size.width - 10,
                                                         0,0)];
}
...
- (void)addShadow{
    ...
    [self.layer setShadowOffset:CGSizeMake(0.5, 5)];
    [self.layer setShadowColor:[[UIColor blackColor] CGColor]];
    [self.layer setShadowOpacity:0.5];
    self.layer.cornerRadius = 4.0f;
    self.layer.shadowRadius = 5.0f;
    self.layer.masksToBounds = NO;
    UIBezierPath *shadowPath = [UIBezierPath bezierPathWithRoundedRect:self.layer.bounds cornerRadius:4.0f];

...
//        self.themeTopSpace.constant -= 10;
    ...
    else
    {
        ...
        for (NSInteger i = 0; i < self.sponsoredRaces.count; i++)
        {
            ...
            height += screenWidth * heightScale;
            
            if (i == 0)
            {
                [self setupConstraintsForSponsoredRaceView:raceListView previousView:nil isFirstView:true parentView:self.sponsorRacesView height:screenWidth * heightScale];
            }
            else
            {
                UIView *previousSponsoredView = [sponsoredViews objectAtIndex: i - 1];
                [self setupConstraintsForSponsoredRaceView:raceListView previousView:previousSponsoredView isFirstView:false parentView:self.sponsorRacesView height:screenWidth * heightScale];
            }
...
//        self.themeTopSpace.constant -= 10;
    ...
    else
    {
        ...
        for (NSInteger i = 0; i < self.exclusiveRaces.count; i++)
        {
            ...
            CGFloat heightScale = 1;
            exclusiveViewHeight += screenWidth * heightScale;
            ...
            if (i == 0)
            {
                [self setupConstraintsForSponsoredRaceView:raceListView previousView:nil isFirstView:true parentView:self.exclusiveRacesView height:screenWidth * heightScale];
            }
            else
            {
                UIView *previousSponsoredView = [exclusiveViews objectAtIndex: i - 1];
                [self setupConstraintsForSponsoredRaceView:raceListView previousView:previousSponsoredView isFirstView:false parentView:self.exclusiveRacesView height:screenWidth * heightScale];
            }
...
//            view.trophyImageView.image = [UIImage imageNamed:@"Icon_Trophy_Gold"];
    ...
    if (race.avatar != [NSNull null] && race.avatar.length)
    {
        [view.avatar setImageWithURL:[NSURL URLWithString:race.avatar]];
    }
    else

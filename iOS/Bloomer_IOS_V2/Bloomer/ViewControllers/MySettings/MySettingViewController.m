...
#if EXTERNALSERVER == TRUE
     #if AMAZON_SERVER == FALSE
    [_lblVersion setText:[NSString stringWithFormat:@"Staging v%@b%@",version, bundle]];
     #else
    [_lblVersion setText:[NSString stringWithFormat:@"LIVE v%@b%@",version, bundle]];
    #endif
#else
    [_lblVersion setText:[NSString stringWithFormat:@"Dev v%@b%@",version, bundle]];
#endif
...
- (IBAction)touchButtonLogOut:(id)sender {
    ...
    if([profileSetting.mobile isEqual:[NSNull null]] || [profileSetting.mobile isEqualToString:@""])
    {
        [self Logout];
        return;
    }
